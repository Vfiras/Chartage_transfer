...
class AdminBooking {
  ...
  factory AdminBooking.fromTrip(TransportTrip trip) {
    final status = switch (trip.status) {
      'requested' => AdminBookingStatus.pending,
      'assigned' ||
      'started' ||
      'arrived' ||
      'contacted' =>
        AdminBookingStatus.confirmed,
      'completed' => AdminBookingStatus.completed,
      'rejected' => AdminBookingStatus.cancelled,
      _ => AdminBookingStatus.pending,
    };
    return AdminBooking(
      reference: trip.id,
      pickupDateTime: trip.pickupTime,
      source: trip.pickupLocation,
      destination: trip.destinationName,
      supplierName: trip.driverId ?? 'Unassigned',
      price: trip.earningsLabel,
      paymentMethod: 'Operations',
      clientName: trip.passengerName,
      clientPhone: trip.passengerPhone,
      route: '${trip.pickupLocation} to ${trip.destinationName}',
      statusText: trip.status,
      notes: 'Synced from MongoDB trips collection.',
      status: status,
    );
  }
  ...
  static const sampleData = [
    AdminBooking(
      ...
      notes:
          'VIP airport pickup with luggage assistance and meet-and-greet service.',
